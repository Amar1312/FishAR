...
public class ReadQRCode : MonoBehaviour
{
    ...
    public TextMeshProUGUI _qrCode;
    ...
    public void Start()
    {
        ...
        InvokeRepeating(nameof(ScanQR), 1f, 1f);
    }
        ...
        if (grabQR)
        {
            ...
            try
            {
                ...
                if (result != null)
                {
                    ...
                    _qrCode.text = result.Text;
                    QRManager.Instance.QRReadText(result.Text);
                }
                else
                {
                    //QRManager.Instance._qrAddPanel.OpenMessagePanel("NO_QR_MESSAGE");
                }
            }
            catch (Exception e)
            {
